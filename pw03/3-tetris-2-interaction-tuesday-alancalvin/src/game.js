...
export class Game extends Map {
  ...
  moveShape(id, col) {
    ...
    if(this.map.testShape(shape, shape.row, col, shape.rotation)) shape.col = col;
  }
  ...
  rotateShape(id, rotation) {
    ...
    if (rotation === "left") {
      if (this.map.testShape(shape, shape.row, shape.col, (shape.rotation + 3) % 4)) shape.rotation = (shape.rotation + 3) % 4;
    }
    else if (rotation === "right") {
      if (this.map.testShape(shape, shape.row, shape.col, (shape.rotation + 1) % 4)) shape.rotation = (shape.rotation + 1) % 4;
    }
  }
  ...
  onMessage(playerId, message) {
    // TODO
    if(message instanceof MoveMessage) this.moveShape(playerId, message.getCol());
    else if(message instanceof RotateMessage) this.rotateShape(playerId, message.getDirection());
    else if(message instanceof DropMessage) this.dropShape(playerId);
  }
}
